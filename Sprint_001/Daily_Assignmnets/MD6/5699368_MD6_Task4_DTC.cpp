...
// Default constructors --> using the concept of constructor delegation --> assigning code to null so that it doesnt cause any issue with virtual inheritance
DTC::DTC() : DTC("","",false,0,0,0,false,false,false,"","") {}

// Parameterized constructor --> using the initializer list
DTC::DTC(const std::string& code_,const std::string& description_,bool active_,int occurenceCount_,std::time_t lastOccurenceTime_,int severityCode_,bool confirmed_,bool pending_,bool milOn_,const std::string& system_,const std::string& origin_): m_description(description_), m_active(active_), m_occurenceCount(occurenceCount_),m_confirmed(confirmed_),m_pending(pending_),m_milOn(milOn_),m_system(system_),m_origin(origin_) {
    ...
    // Hadling severity 
    if(severityCode_ >=0 && severityCode_<=2) {m_severityLevel =  static_cast<SeverityLevel>(severityCode_);}
    else {throw std::invalid_argument("InvalidSeverityException: \"Severity level must be between 0 (INFO) and 2 (CRIT)\"");}
...
// copy constructor
DTC::DTC(const DTC& other) : DTC(other.m_code,other.m_description,other.m_active,other.m_occurenceCount,other.m_lastOccurenceTime,static_cast<unsigned int>(other.m_severityLevel),other.m_confirmed,other.m_pending,other.m_milOn,other.m_system,other.m_origin){}
