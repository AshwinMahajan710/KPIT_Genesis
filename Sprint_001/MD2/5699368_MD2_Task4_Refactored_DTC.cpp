...
#include "5699368_MD2_Task4_Refactored_DTC.h"
#include <iostream>
#include <ctime>
#include <string>
...
// Getters (added const for MISRA compliance)
...
// Getter for static DTC count
int DTC::getDTCCount() { return s_DTCCount; }

// Setters (removed const on small types, added const & for strings)
...
// Print info (added const for MISRA)
...
// Return string severity (const added)
...
// Compute risk score (const added)
double DTC::computeRiskScore() const
{
    ...
    double occurenceComponent_ = (m_occurenceCount <= 0) ? 0.0 : std::min(50.0, m_occurenceCount * 5.0);
    ...
    totalScore_ = std::max(0.0, std::min(totalScore_, 100.0));
    return totalScore_;
}

// Recommend action (const added)
...
// Merge if same code (const & added)
bool DTC::mergeIfSameCode(const DTC &other)
{
    if ((getCode() == other.getCode()) && (getSystem() == other.getSystem()))
    {
        ...
        if ((getOrigin().length() == 0) && (other.getOrigin().length() != 0)) { setOrigin(other.getOrigin()); }
        if ((getDescription().length() == 0) && (other.getDescription().length() != 0)) { setDescription(other.getDescription()); }
