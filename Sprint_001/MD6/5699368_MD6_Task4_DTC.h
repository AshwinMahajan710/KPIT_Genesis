...
class DTC{
    ...
    private:
        ...
        std::string m_system;
        std::string m_origin;
        const int m_maxSeverity = 2; // setting up max_severity so that we can use this to take validity
    ...
    public:
        ...
        // constructors 
        DTC();
        DTC(const std::string& code_,const std::string& description_,bool active_,int occurenceCount_,std::time_t lastOccurenceTime_,int severityCode_,bool confirmed_,bool pending_,bool milOn_,const std::string& system_,const std::string& origin_);
        DTC(const DTC& other);
