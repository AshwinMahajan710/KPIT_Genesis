...
class MyClass{
    private:
        ...
        int * ptr;
    public:
        MyClass() : myIntVar(0), myDoubleVar (0.0), mystringVar(""),ptr(nullptr){};

        // move constructor
        MyClass(MyClass&& other) : myIntVar(other.myIntVar), myDoubleVar(other.myDoubleVar), mystringVar(other.mystringVar), ptr(other.ptr){}

        // dispplay function
        void display(){
            std::cout<<"Int Variable: "<<myIntVar<<std::endl;
            std::cout<<"Double Variable: "<<myDoubleVar<<std::endl;
            std::cout<<"String Variable: "<<mystringVar<<std::endl;
            if(ptr == nullptr){
                std::cout<<"ptr: nullptr"<<std::endl;
            }
            else{
                std::cout<<"ptr"<<myIntVar<<std::endl;
            }
        }
};
